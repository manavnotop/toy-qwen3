...
def compute_rope_params(head_dims, theta_base=10000, context_length=4096, dtype=torch.float32):
    ...
    inv_freq = 1.0 / (
        theta_base ** (torch.arange(0, head_dims, 2, dtype=dtype) / head_dims)
    )  # -> (head_dims/2,)
...
def apply_rope(x, cos, sin):
    ...
    )  # (seq_length, head_dim) -> (1, 1, seq_length, head_dim)
...
class GroupQueryAttention(nn.Module):
    ...
    def forward(self, x, mask, cos, sin):
        ...
        values = values.view(b, num_tokens, self.num_kv_groups, self.head_dim).transpose(
            1, 2
        )  # -> (b, num_kv_groups, num_tokens, head_dim)
